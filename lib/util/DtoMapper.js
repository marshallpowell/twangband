...
var SmallSongSearchResultDto = require(global.PUBLIC_APP_LIB+'models/SmallSongSearchResultDto.js');
...
var LikeContentDto = require(global.PUBLIC_APP_LIB+'models/LikeContentDto.js');
...
DtoMapper.mapTrackModel=function(track){
    ...
    dto.tags= track.tags;
    dto.isPublic = track.isPublic;
    dto.songIds = (track.songIds)? track.songIds : [];
    ...
    //logger.debug('trackDto: ' + JSON.stringify(dto));
    return dto;
};

DtoMapper.mapSmallSongSearchResultModel=function(smallSong){

    var dto = new SmallSongSearchResultDto();
    dto.id = smallSong.id;
    dto.name = smallSong.name;
    dto.creatorId = smallSong.creator;
    dto.fileName = smallSong.fileName;
    return dto;
};
...
DtoMapper.mapUserModel=function(user){
    ...
    userDto.id = user._id;
    userDto.uiId = '_'+user._id;
    userDto.profilePic = user.profilePic;
    userDto.instruments = user.instruments;
    userDto.tags = user.tags;

    if(user.facebookId){
        userDto.loginType = AppConstants.LOGIN_TYPES.FB;
    }
    //twangband
    else{
        userDto.loginType = AppConstants.LOGIN_TYPES.TB;
    }

    return userDto;
};

DtoMapper.mapLikeContentModel = function(content, count){

    var likeDto = new LikeContentDto(content.creator, true, content.entity, content.entityType,count,content.dateCreated);
    return likeDto;
