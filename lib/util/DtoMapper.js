...
DtoMapper.mapSongModel=function(song){
    ...
    for(var i = 0; i < song.songTracks.length; i++){
       ...
       // log.debug("mapping song track " + JSON.stringify(songTrackDto) + " from " + JSON.stringify(song.songTracks[i]));
...
DtoMapper.mapUserModel=function(user){
    ...
    //log.debug("mapping user: " + JSON.stringify(user));
    ...
    //log.debug("mapped user to dto: " + JSON.stringify(userDto));
