...
AuthUtils.doPassportSignin = function(user) {
    ...
    log.debug('create cookie and sign in for: ' + JSON.stringify(user));
        ...
        if(user.firstLogin){
            passport.req.login(user, function(err){
                if(err){
                    ...
                    log.debug("login called, first timer, redirect to /user/profile ");
                    passport.res.redirect('/user/profile');
                }
