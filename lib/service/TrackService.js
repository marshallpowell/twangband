...
TrackService.callMergeTracks = function(trackDtos){

    if(global.ENV != 'local'){
        return TrackService.mergeTracks(trackDtos);
    }
    ...
    try {
        ...
        var req = http.request(options, function (res) {
            ...
            res.on('end', function () {
                log.debug('finished merging tracks json response: ' + jsonResponse);
                var trackData = JSON.parse(jsonResponse);
                trackData.name=songFileName;
...
TrackService.mergeTracks=function(trackDtos){
    ...
    var ffmpegMergeCMD = global.FFMPEG + ' ' +inputTracksCmd+' -filter_complex amix=inputs='+data.tracks.length+' -t '+duration +' '+global.UPLOADS_DIR+data.name;
    log.debug('ffmpegMergeCMD: ' + ffmpegMergeCMD);
    var ffprobeCmd = global.FFPROBE+' -v quiet -print_format json -show_format -show_streams '+global.UPLOADS_DIR+data.name;
...
TrackService.saveNewTracks = function(reqFiles, songDto, userDto){
    ...
    if(updatedTracksSize > 0 && fileArrayLength==0){
        ...
        TrackService.callMergeTracks(tracksToKeep).then(function (mediaDataDto) {
