...
//FIXME this only works if you are uploading one file at a time, otherwise the index gets updated before this function finishes.
                    ...
                    if(counter == fileArrayLength){
                        ...
                        if(songDto.tracks.length > 1){
                            TrackService.mergeTracks(songDto).then(function(audioDataDto){
                                deferred.resolve(songDto);
                            });
                        }
                        else{
