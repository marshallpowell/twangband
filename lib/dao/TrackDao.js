...
TrackDao.findUserTracks = function(userDto){

    log.debug('enter findUserTracks');
    ...
    Track.find(query, function (err, tracks) {
        ...
        if (err) {
            ...
            logger.debug('found tracks: ' + tracks.length);
            var dtos = [];
            for(var i = 0; i < tracks.length; i++){
                dtos.push(dtoMapper.mapTrackModel(tracks[i]));
            }
...
TrackDao.updateSearch = function(trackDto){
    ...
    Track.findOne(query)
        .exec(function (err, track) {
            ...
            var songIds = (track.songIds) ? track.songIds : [];

            track.songKeywords = _.union(songKeywords, trackDto.songKeywords);
            track.songIds = _.union(songIds, trackDto.songIds);
